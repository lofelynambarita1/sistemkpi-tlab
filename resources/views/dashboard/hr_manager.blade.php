@extends('layouts.app')
@section('title', 'Dashboard HR/Manager')
@section('content')
...
<header class="mb-6">
    ...
    <p class="text-gray-600">Kelola &amp; Review Dokumen KPI Staff &middot; Periode {{ $year }}</p>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
    ...
    <div class="bg-white rounded-lg shadow p-6 border-l-4 border-yellow-500">
        ...
        <p class="text-3xl font-bold text-gray-800 mt-1">{{ $stats['submitted'] }}</p>
    ...
    <div class="bg-white rounded-lg shadow p-6 border-l-4 border-cyan-500">
        ...
        <p class="text-3xl font-bold text-gray-800 mt-1">{{ $stats['reviewed'] }}</p>
...
<div class="bg-white rounded-lg shadow p-6 mb-6">
    <div class="flex items-center justify-between mb-4">
        ...
        <a href="{{ route('hr.kpi.index') }}" class="btn-secondary">Lihat Semua</a>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-4 py-3 text-sm"><span class="badge badge-diproses">{{ $doc->user->role_label }}</span></td>
                    <td class="px-4 py-3 text-sm text-gray-600">{{ $doc->period_year }}</td>
                    <td class="px-4 py-3 text-sm"><span class="badge badge-{{ $doc->status }}">{{ $doc->status_label }}</span></td>
                    <td class="px-4 py-3 text-sm font-semibold text-red-700">{{ number_format($doc->total_score, 2) }}</td>
                    <td class="px-4 py-3 text-sm text-gray-500">{{ $doc->updated_at->diffForHumans() }}</td>
                    <td class="px-4 py-3 text-sm">
                        <div class="flex gap-1">
                            <a href="{{ route('hr.kpi.show', $doc->id) }}" class="btn-secondary text-xs" style="padding:0.25rem 0.5rem;" title="Lihat">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                            </a>
                            <a href="{{ route('hr.kpi.edit', $doc->id) }}" class="btn-secondary text-xs" style="padding:0.25rem 0.5rem;" title="Edit">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </a>
                            <button class="btn-secondary text-xs" style="padding:0.25rem 0.5rem;color:#dc2626;" data-delete-url="{{ route('hr.kpi.destroy', $doc->id) }}" data-delete-desc="Dokumen KPI milik {{ $doc->user->name }} ({{ $doc->period_year }}) akan dihapus." title="Hapus">
