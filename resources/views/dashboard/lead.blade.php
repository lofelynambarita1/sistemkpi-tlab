...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
    <div class="bg-white rounded-lg shadow p-6 border-l-4 border-blue-500">
        ...
        <p class="text-3xl font-bold text-gray-800 mt-1">{{ $totalEmployee }}</p>
        <p class="text-sm text-gray-400">Bawahan yang diawasi</p>
    </div>
    <div class="bg-white rounded-lg shadow p-6 border-l-4 border-yellow-500">
        <p class="text-sm text-gray-500">KPI Menunggu Review</p>
        <p class="text-3xl font-bold text-gray-800 mt-1">{{ $totalSubmitted }}</p>
...
<div class="bg-white rounded-lg shadow p-6 mb-6">
    ...
    <p class="text-gray-600 mb-4">
        Saat ini <strong class="text-yellow-600">{{ $totalSubmitted }}</strong> dokumen KPI sedang menunggu review.
    </p>
    <div class="flex flex-wrap gap-3">
        @if($totalSubmitted > 0)
        <a href="{{ route('review.index') }}" class="btn-primary">Review Sekarang</a>
        @endif
        <a href="{{ route('kpi.create') }}" class="btn-secondary">Isi Form KPI</a>
        <a href="{{ route('kpi.index') }}" class="btn-secondary">History KPI</a>
    </div>
</div>
@endsection
