...
<nav class="mb-4 text-sm">
    <ol class="flex text-gray-500 gap-2">
        ...
        <li>/</li>
        <li class="text-gray-700">Review KPI — {{ $kpiDocument->user->name }}</li>
    </ol>
</nav>

<header class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
    <div>
        <h1 class="text-2xl font-bold text-gray-800">Review KPI — {{ $kpiDocument->user->name }} ({{ $kpiDocument->period_year }})</h1>
        <p class="text-gray-600 text-sm">
            {{ $kpiDocument->user->role_label ?? '' }} · Dibuat: {{ $kpiDocument->created_at->format('d M Y') }}
    ...
    <div class="flex gap-2 flex-wrap">
        <a href="{{ route('hr.kpi.index') }}" class="btn-secondary">Kembali</a>
        <a href="{{ route('hr.kpi.edit', $kpiDocument->id) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded text-sm font-medium">Edit</a>
        <button type="button" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded text-sm font-medium"
                ...
                data-delete-desc="Dokumen KPI milik {{ $kpiDocument->user->name }} ({{ $kpiDocument->period_year }}) akan dihapus permanen.">Hapus</button>
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
    <div class="bg-white rounded-lg shadow p-4 text-center flex flex-col items-center justify-center">
        <span class="badge {{ $kpiDocument->status_badge_class }} text-sm px-3 py-2">{{ $kpiDocument->status_label }}</span>
    ...
    <div class="md:col-span-3 bg-white rounded-lg shadow p-4">
        <div class="grid grid-cols-2 md:grid-cols-6 gap-2 text-center">
            <div class="border-r">
                ...
                <div class="font-bold text-red-700 text-lg">{{ number_format($jdTotal,2) }}</div>
            </div>
            <div class="border-r">
                <div class="text-muted small text-gray-500 text-xs">CI</div>
                <div class="font-bold text-green-600 text-lg">{{ number_format($ciTotal,2) }}</div>
            </div>
            <div class="border-r">
                <div class="text-muted small text-gray-500 text-xs">Self Dev</div>
                <div class="font-bold text-yellow-600 text-lg">{{ number_format($sdTotal,2) }}</div>
            </div>
            <div class="border-r">
                <div class="text-muted small text-gray-500 text-xs">HR Act</div>
                <div class="font-bold text-purple-600 text-lg">{{ number_format($hrTotal,2) }}</div>
            </div>
            <div class="border-r">
                <div class="text-muted small text-gray-500 text-xs">Perilaku</div>
                <div class="font-bold text-cyan-600 text-lg">{{ number_format($pkTotal,2) }}</div>
            </div>
            <div>
                <div class="text-muted small text-gray-500 text-xs font-semibold">TOTAL</div>
                <div class="font-bold text-2xl text-gray-800">{{ number_format($kpiDocument->total_score,2) }}</div>
            </div>
        </div>
    </div>
</div>
...
<div class="border border-gray-200 rounded-t-lg bg-gray-50 px-4 pt-3">
    <div class="flex flex-wrap gap-1">
        ...
        <a class="px-4 py-2 text-sm font-medium text-gray-600 hover:text-red-700" data-bs-toggle="tab" href="#hr-history">
            History
            <span class="badge badge-diproses ms-1">{{ $kpiDocument->histories->count() }}</span>
        </a>
    </div>
</div>
            ...
            <table class="min-w-full divide-y divide-gray-200 subform-view-table">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-3 py-2 text-sm calc-cell">{{ number_format($jd->jumlah_koefisien,2) }}</td>
                        <td class="px-3 py-2 text-sm calc-cell">{{ number_format($jd->total_mandays_penugasan,2) }}</td>
                        <td class="px-3 py-2 text-center">
                            <button class="text-yellow-600 hover:text-yellow-800 text-sm" onclick="toggleEdit('jd{{ $jd->id }}')">Edit</button>
                        </td>
                    </tr>
                    <tr id="jd{{ $jd->id }}-row" style="display:none;">
                        <td colspan="8" class="p-0">
                            <form method="POST" action="{{ route('hr.kpi.update.jobdesc', [$kpiDocument->id, $jd->id]) }}" class="p-3 bg-gray-50 border-t">
                                @csrf @method('PUT')
                                <div class="grid grid-cols-2 md:grid-cols-5 gap-2">
                                    <div>
                                        <label class="block text-xs text-gray-600 mb-1">Koef. OTB</label>
                                        <input type="number" name="penilaian_koefisien_ontime_onbudget" class="w-full px-2 py-1 border border-gray-300 rounded text-sm" value="{{ $jd->penilaian_koefisien_ontime_onbudget }}" step="0.01" min="0">
                                    </div>
                                    <div>
                                        <label class="block text-xs text-gray-600 mb-1">Grade Project</label>
                                        <input type="number" name="penilaian_grade_project" class="w-full px-2 py-1 border border-gray-300 rounded text-sm" value="{{ $jd->penilaian_grade_project }}" step="0.01" min="0">
                                    </div>
                                    <div class="md:col-span-2">
                                        <label class="block text-xs text-gray-600 mb-1">Nama Kegiatan & Bukti</label>
                                        <input type="text" name="nama_kegiatan_bukti" class="w-full px-2 py-1 border border-gray-300 rounded text-sm" value="{{ $jd->nama_kegiatan_bukti }}">
                                    </div>
                                    <div>
                                        <label class="block text-xs text-gray-600 mb-1">Mandays Proyek</label>
                                        <input type="number" name="mandays_proyek" class="w-full px-2 py-1 border border-gray-300 rounded text-sm" value="{{ $jd->mandays_proyek }}" step="0.01" min="0">
                                    </div>
                                </div>
                                <div class="flex gap-2 mt-2">
                                    <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded text-sm">Simpan</button>
                                    <button type="button" class="btn-secondary py-1 px-3 text-sm" onclick="toggleEdit('jd{{ $jd->id }}')">Batal</button>
                                </div>
                            </form>
                        </td>
                    </tr>
                ...
                <tfoot class="bg-red-50">
                    <tr><td colspan="6" class="px-3 py-2 text-right font-bold text-sm">Total:</td><td class="px-3 py-2 font-bold text-sm">{{ number_format($jdTotal,2) }}</td><td></td></tr>
                </tfoot>
            </table>
        </div>
        @endif
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 subform-view-table">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        <td class="px-3 py-2 text-sm">{{ $i+1 }}</td>
                        <td class="px-3 py-2"><span class="badge badge-diproses">{{ $ci->jenis_kegiatan_bukti }}</span></td>
                        <td class="px-3 py-2 text-sm">{{ $ci->kegiatan }}</td>
                        <td class="px-3 py-2 text-sm">{{ number_format($ci->mandays,2) }}</td>
                        <td class="px-3 py-2 text-sm calc-cell">{{ number_format($ci->koefisien,4) }}</td>
                        <td class="px-3 py-2 text-sm calc-cell">{{ number_format($ci->point,4) }}</td>
                        <td class="px-3 py-2 text-center">
                            <button class="text-yellow-600 hover:text-yellow-800 text-sm" onclick="toggleEdit('ci{{ $ci->id }}')">Edit</button>
                        </td>
                    </tr>
                    <tr id="ci{{ $ci->id }}-row" style="display:none;">
                        <td colspan="7" class="p-0">
                            <form method="POST" action="{{ route('hr.kpi.update.ci', [$kpiDocument->id, $ci->id]) }}" class="p-3 bg-gray-50 border-t">
                                @csrf @method('PUT')
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                                    ...
                                    <div class="md:col-span-2">
                                        ...
                                        <input type="text" name="kegiatan" class="w-full px-2 py-1 border border-gray-300 rounded text-sm" value="{{ $ci->kegiatan }}">
                                    </div>
                                    <div>
                                        <label class="block text-xs text-gray-600 mb-1">Mandays</label>
                                        <input type="number" name="mandays" class="w-full px-2 py-1 border border-gray-300 rounded text-sm" value="{{ $ci->mandays }}" step="0.01" min="0">
                                    </div>
                                </div>
                                <div class="flex gap-2 mt-2">
                                    <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded text-sm">Simpan</button>
                                    <button type="button" class="btn-secondary py-1 px-3 text-sm" onclick="toggleEdit('ci{{ $ci->id }}')">Batal</button>
                                </div>
                            </form>
                        </td>
                    </tr>
                ...
                <tfoot class="bg-green-50">
                    <tr><td colspan="5" class="px-3 py-2 text-right font-bold text-sm">Total Point:</td><td class="px-3 py-2 font-bold text-sm">{{ number_format($ciTotal,4) }}</td><td></td></tr>
                </tfoot>
            </table>
        </div>
        @endif
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 subform-view-table">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        <td class="px-3 py-2 text-sm">{{ $i+1 }}</td>
                        <td class="px-3 py-2"><span class="badge badge-draft">{{ $sd->jenis_sd }}</span></td>
                        <td class="px-3 py-2 text-sm">{{ $sd->kegiatan }}</td>
                        <td class="px-3 py-2 text-sm">{{ number_format($sd->mandays,2) }}</td>
                        <td class="px-3 py-2 text-sm calc-cell">{{ number_format($sd->koefisien,4) }}</td>
                        <td class="px-3 py-2 text-sm calc-cell">{{ number_format($sd->point,4) }}</td>
                        <td class="px-3 py-2 text-center">
                            <button class="text-yellow-600 hover:text-yellow-800 text-sm" onclick="toggleEdit('sd{{ $sd->id }}')">Edit</button>
                        </td>
                    </tr>
                    <tr id="sd{{ $sd->id }}-row" style="display:none;">
                        <td colspan="7" class="p-0">
                            <form method="POST" action="{{ route('hr.kpi.update.sd', [$kpiDocument->id, $sd->id]) }}" class="p-3 bg-gray-50 border-t">
                                @csrf @method('PUT')
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                                    <div>
                                    ...
                                    <div class="md:col-span-2">
                                        <label class="block text-xs text-gray-600 mb-1">Kegiatan</label>
                                        <input type="text" name="kegiatan" class="w-full px-2 py-1 border border-gray-300 rounded text-sm" value="{{ $sd->kegiatan }}">
                                    </div>
                                    <div>
                                        <label class="block text-xs text-gray-600 mb-1">Mandays</label>
                                        <input type="number" name="mandays" class="w-full px-2 py-1 border border-gray-300 rounded text-sm" value="{{ $sd->mandays }}" step="0.01" min="0">
                                    </div>
                                </div>
                                <div class="flex gap-2 mt-2">
                                    <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded text-sm">Simpan</button>
                                    <button type="button" class="btn-secondary py-1 px-3 text-sm" onclick="toggleEdit('sd{{ $sd->id }}')">Batal</button>
                                </div>
                            </form>
                        </td>
                    </tr>
                ...
                <tfoot class="bg-yellow-50">
                    <tr><td colspan="5" class="px-3 py-2 text-right font-bold text-sm">Total Point:</td><td class="px-3 py-2 font-bold text-sm">{{ number_format($sdTotal,4) }}</td><td></td></tr>
                </tfoot>
            </table>
        </div>
        @endif
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 subform-view-table">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        <td class="px-3 py-2 text-sm">{{ $i+1 }}</td>
                        <td class="px-3 py-2"><span class="badge badge-diproses">{{ $hr->jenis_kegiatan }}</span></td>
                        <td class="px-3 py-2 text-sm">{{ $hr->kegiatan }}</td>
                        <td class="px-3 py-2 text-sm">{{ number_format($hr->mandays,2) }}</td>
                        <td class="px-3 py-2 text-sm calc-cell">{{ number_format($hr->koefisien,4) }}</td>
                        <td class="px-3 py-2 text-sm calc-cell">{{ number_format($hr->point,4) }}</td>
                        <td class="px-3 py-2 text-center">
                            <button class="text-yellow-600 hover:text-yellow-800 text-sm" onclick="toggleEdit('hr{{ $hr->id }}')">Edit</button>
                        </td>
                    </tr>
                    <tr id="hr{{ $hr->id }}-row" style="display:none;">
                        <td colspan="7" class="p-0">
                            <form method="POST" action="{{ route('hr.kpi.update.hr_activity', [$kpiDocument->id, $hr->id]) }}" class="p-3 bg-gray-50 border-t">
                                @csrf @method('PUT')
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                                    <div>
                                    ...
                                    <div class="md:col-span-2">
                                        <label class="block text-xs text-gray-600 mb-1">Kegiatan</label>
                                        <input type="text" name="kegiatan" class="w-full px-2 py-1 border border-gray-300 rounded text-sm" value="{{ $hr->kegiatan }}">
                                    </div>
                                    <div>
                                        <label class="block text-xs text-gray-600 mb-1">Mandays</label>
                                        <input type="number" name="mandays" class="w-full px-2 py-1 border border-gray-300 rounded text-sm" value="{{ $hr->mandays }}" step="0.01" min="0">
                                    </div>
                                </div>
                                <div class="flex gap-2 mt-2">
                                    <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded text-sm">Simpan</button>
                                    <button type="button" class="btn-secondary py-1 px-3 text-sm" onclick="toggleEdit('hr{{ $hr->id }}')">Batal</button>
                                </div>
                            </form>
                        </td>
                    </tr>
                ...
                <tfoot style="background:#ede9fe;">
                    <tr><td colspan="5" class="px-3 py-2 text-right font-bold text-sm">Total Point:</td><td class="px-3 py-2 font-bold text-sm">{{ number_format($hrTotal,4) }}</td><td></td></tr>
                </tfoot>
            </table>
        </div>
        @endif
            ...
            <table class="min-w-full divide-y divide-gray-200 subform-view-table">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-3 py-2 text-center">
                            <span class="font-bold text-lg {{ $kp->score >= $kp->minimum_capaian ? 'text-green-600' : 'text-red-600' }}">
                        ...
                        <td class="px-3 py-2 text-center">
                            <button class="text-yellow-600 hover:text-yellow-800 text-sm" onclick="toggleEdit('kp{{ $kp->id }}')">Edit</button>
                        </td>
                    </tr>
                    <tr id="kp{{ $kp->id }}-row" style="display:none;">
                        <td colspan="9" class="p-0">
                            <form method="POST" action="{{ route('hr.kpi.update.perilaku', [$kpiDocument->id, $kp->id]) }}" class="p-3 bg-gray-50 border-t">
                                @csrf @method('PUT')
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-2 items-end">
                                    <div>
                                        <label class="block text-xs text-gray-600 mb-1">Score (0-100)</label>
                                        <input type="number" name="score" class="w-full px-2 py-1 border border-gray-300 rounded text-sm" value="{{ $kp->score }}" min="0" max="100" step="0.01">
                                    </div>
                                    <div class="flex gap-2">
                                        <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded text-sm">Simpan</button>
                                        <button type="button" class="btn-secondary py-1 px-3 text-sm" onclick="toggleEdit('kp{{ $kp->id }}')">Batal</button>
                                    </div>
                                </div>
                            </form>
                        </td>
                ...
                <tfoot class="bg-cyan-50">
                    <tr><td colspan="6" class="px-3 py-2 text-right font-bold text-sm">Total Score Perilaku:</td><td class="px-3 py-2 text-center font-bold text-lg">{{ number_format($pkTotal,2) }}</td><td colspan="2"></td></tr>
                </tfoot>
            </table>
        </div>
        @endif
    ...
    <div class="tab-pane fade" id="hr-history">
        ...
        @else
            ...
            <div class="border-l-4 mb-4 pl-4 py-2
                @if($hist->action === 'create') border-green-500
                @elseif($hist->action === 'update') border-yellow-500
                @elseif($hist->action === 'delete') border-red-500
                @elseif($hist->action === 'submit') border-blue-500
                @else border-gray-500 @endif">
                <div class="flex items-start justify-between flex-wrap gap-2 mb-1">
                    <div>
                        <span class="badge
                            @if($hist->action === 'create') badge-aktif
                            @elseif($hist->action === 'update') badge-draft
                            @elseif($hist->action === 'delete') badge-dicabut
                            @elseif($hist->action === 'submit') badge-diproses
                            @else badge-expired @endif">{{ ucfirst($hist->action) }}</span>
                        @if($hist->section)
                            <span class="bg-gray-100 text-gray-700 px-2 py-0.5 rounded text-xs">{{ $hist->section_label }}</span>
                        @endif
                        <span class="text-sm text-gray-700">{{ $hist->description }}</span>
                    </div>
                    <small class="text-gray-500">{{ $hist->created_at->format('d M Y H:i') }}</small>
                </div>
                ...
                <div class="mt-1">
                    <a class="text-blue-600 hover:underline text-xs" data-bs-toggle="collapse" href="#hd{{ $hist->id }}">Lihat Detail</a>
                    <div class="collapse mt-2" id="hd{{ $hist->id }}">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                            <div>
                                <small class="text-gray-500 font-semibold d-block mb-1">Sebelum:</small>
                                <pre class="bg-gray-50 rounded p-2 text-xs" style="max-height:150px;overflow:auto;">{{ json_encode($hist->old_data, JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE) }}</pre>
                            </div>
                            <div>
                                <small class="text-gray-500 font-semibold d-block mb-1">Sesudah:</small>
                                <pre class="bg-gray-50 rounded p-2 text-xs" style="max-height:150px;overflow:auto;">{{ json_encode($hist->new_data, JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE) }}</pre>
                            </div>
                        </div>
                    </div>
                </div>
