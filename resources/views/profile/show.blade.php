...
<div class="bg-white rounded-xl shadow p-6 max-w-2xl">
    <div class="flex items-center gap-4 mb-6">
        ...
        <div>
            ...
            <span class="bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-full text-xs">{{ ucfirst(str_replace('_', ' ', $user->role)) }}</span>
        </div>
    </div>
    <form method="POST" action="{{ request()->route()->getPrefix() ? url(request()->route()->getPrefix() . '/profile') : route('admin.profile.update') }}">
        @csrf @method('PUT')
        <div class="grid grid-cols-1 gap-4">
            ...
            <div><label class="block text-sm font-medium text-gray-700 mb-1">Email</label>
                <input type="text" value="{{ $user->email }}" disabled class="w-full border border-gray-200 bg-gray-50 rounded-lg px-3 py-2 text-sm text-gray-500">
            </div>
            ...
            <div><label class="block text-sm font-medium text-gray-700 mb-1">Role</label>
                <input type="text" value="{{ ucfirst(str_replace('_', ' ', $user->role)) }}" disabled class="w-full border border-gray-200 bg-gray-50 rounded-lg px-3 py-2 text-sm text-gray-500">
            </div>
            <hr class="my-2">
